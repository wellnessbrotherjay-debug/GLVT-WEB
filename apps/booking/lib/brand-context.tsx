...
export function BrandProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchTheme = useCallback(async (slug: string) => {
    ...
    if (!data) {
      await (supabase.from("brand_settings") as any).upsert(themeToRow(defaultTheme, slug), { onConflict: "slug" });
      applyTheme(defaultTheme, slug);
      if (slugRef.current === slug) setLoading(false);
      return;
    }
  ...
  const saveTheme = useCallback(
    async (next: BrandTheme) => {
      ...
      const { error } = await (supabase.from("brand_settings") as any).upsert(themeToRow(next, targetSlug), { onConflict: "slug" }).select();
      if (error) {
  ...
  const value = useMemo(
    () => ({
      ...
      loading,
      error,
      refresh: () => fetchTheme(derivedSlug),
      saveTheme,
      patchTheme,
      resetTheme,
    }),
    [theme, loading, error, fetchTheme, saveTheme, patchTheme, resetTheme, derivedSlug]
  );
