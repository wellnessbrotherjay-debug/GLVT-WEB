...
// @ts-ignore
export type CircuitSession = any;

export const circuitService = {
    // --- 1. SETUP ---

    async createSession(circuitId: string) {
        const { data, error } = await (supabase.from('circuit_sessions') as any)
            .insert({
    ...
    async registerDevice(sessionId: string, deviceType: 'tablet' | 'controller', stationNumber?: number) {
        ...
        await (supabase.from('circuit_devices') as any)
            .insert({
    ...
    async startCircuit(sessionId: string) {
        ...
        await (supabase.from('circuit_sessions') as any)
            .update({
                status: 'active',
